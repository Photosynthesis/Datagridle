...
if(!$_SESSION['auth']['userhash']){
  exit('Content requires authentication');
}

// Random string used to secure Flash upload if session control not enabled - be sure to change!
$tinybrowser['obfuscate'] = '1a7f932be41bd';
...
include_once('../../../../cms_root_path.php');

$tinybrowser['path']['image'] = $cms_root_path.'content/images/'; // Image files location - also creates a '_thumbs' subdirectory within this path to hold the image thumbnails
$tinybrowser['path']['media'] = $cms_root_path.'content/media/'; // Media files location
$tinybrowser['path']['file']  = $cms_root_path.'content/documents/'; // Other files location
